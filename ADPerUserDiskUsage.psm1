    ...
    Process{
        ...
        foreach ($User in $UserList) {
            ...
            If($User.ProfilePath)
            {
                ...
                $ProfileLiteralPathList = Invoke-Command -ComputerName $ProfileComputerName {
                    (Get-SmbShare -Name "$Using:ProfileShareName*").Path
                }
                ...
                If($ProfileLiteralPathList)
                {
                    foreach($ProfileLiteralPath in $ProfileLiteralPathList)
                    {
                        $ProfileSize = Invoke-Command -ComputerName $ProfileComputerName {
                            (Get-ChildItem2 -Path $Using:ProfileLiteralPath -Recurse -Force -ErrorAction SilentlyContinue |
            ...
            If($User.HomeDirectory)
            {
                ...
                $HomeLiteralPath = Invoke-Command -ComputerName $HomeComputerName {
                    (Get-SmbShare -Name "$Using:HomeShareName*").Path
                }

                $HomeSize = Invoke-Command -ComputerName $HomeComputerName {
                            (Get-ChildItem2 -Path $Using:HomeLiteralPath -Recurse -Force -ErrorAction SilentlyContinue |
                                Measure-Object -Property Length -Sum).Sum/1Mb
                        }
